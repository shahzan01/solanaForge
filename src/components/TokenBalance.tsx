...
export function TokenBalance() {
  ...
  useEffect(() => {
    ...
    const getBalances22 = async () => {
      ...
      try {
          ...
          .map(async (accountInfo) => {
            try {
              ...
              if (metadata?.uri.startsWith("ipfs")) {
                const data =
                  await IPFSMetadataService.fetchMetadataWithFallback(
                    metadata.uri
                  );
        ...
        Promise.all(tokens)
          .then((resolvedTokens) => {
            const validTokens: Token22Type[] = resolvedTokens.filter(
              (token): token is Token22Type => token !== null
            );
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      {wallet.publicKey && !normalTokens.length && !tokens22.length && (
        <p className="ml-2">No Tokens Present</p>
      )}
